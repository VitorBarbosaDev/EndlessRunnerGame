...
class Player {
  ...
  checkBounds(delta) {
    ...
    if (this.position.y <= 0) {
      ...
      // Only apply gravity if the player is not at the top of the canvas
    ...
    if (gameStarted) {
      ...
      if (this.position.x < -this.width - obstacleDistance / 2.5) {
        ...
        if (this.type === ObstacleType.TOP) {
          ...
          let topObstacle = obstacles.find(
            obstacle =>
...
function startCountdown() {
  ...
  let countdownInterval = setInterval(() => {
    ...
    countdownDisplay = countdownNumber > 0.5 ? countdownNumber : "";
...
function displayLeaderboard() {
  ...
  topScores.forEach((score, index) => {
    leaderboardContent.innerHTML += `<p>${index + 1}. ${score.name}: ${
      score.score
    }</p>`;
  });
